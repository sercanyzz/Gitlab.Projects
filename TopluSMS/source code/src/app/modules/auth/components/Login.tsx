...
import {isTokenValid, login} from '../core/_requests'
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, {setStatus, setSubmitting}) => {
      ...
      try {
        ...
        saveAuth(auth)
        const user: UserModel = await isTokenValid(auth.token)
        if (user) {
          setCurrentUser(user)
        }
      } catch (error) {
        console.error(error)
        saveAuth(undefined)
        setStatus('Kullanıcı adı / şifre bilgileri hatalı')
        setSubmitting(false)
        setLoading(false)
      }
    },
  ...
  return (
    ...
    >
      ...
      <div className='text-center mb-11'>
        <h1 className='text-dark fw-bolder mb-3'>Giriş Ekranı</h1>
      </div>
      {/* begin::Heading */}
